  ...
  return (
    <div>
      <div className="flex text-[#DB4444] items-center gap-3 md:ml-36 font-semibold pt-20">
        <div className="bg-[#DB4444] rounded py-5 px-2"></div>Our Products
      </div>
      ...
      <div className="grid md:grid-cols-4 grid-cols-1 md:gap-5 gap-7 mx-6">
        {!isLoading || !isFetching
          ? products.map((product) => (
              <OurProduct key={product.id} product={product}></OurProduct>
            ))
      ...
      <div className="flex justify-center pt-16">
        <button className="text-white bg-[#DB4444] rounded py-3 px-12">
          View All Products
        </button>
      </div>
    </div>
