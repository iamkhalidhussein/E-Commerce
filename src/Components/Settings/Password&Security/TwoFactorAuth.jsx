...
const TwoFactorAuth = () => {
  ...
  const handleToggle2FA = (checked) => {
    if (checked) {
      ...
      setTimeout(() => {
        ...
        setIsLoading(false);
        toast.error(
          "Two-factor authentication disabled. Your account is now less secure. We recommend enabling 2FA for better security.",
        );
      }, 1000);
    }
  };
  ...
  const handleVerifyCode = async (e) => {
    ...
    const verifyTotp = async () => {
      const response = await axiosSecure.post("/auth/verify-totp", {
        token: otp,
        secret,
      });
      console.log(response);
    ...
    const save2FaStats = async () => {
      ...
      const save2FaStat = await axiosSecure.patch(
        `/auth/save2fa-status/${user?.email}`,
        status2Fa,
      );
    ...
    <CardHeader>
      <div className="flex items-center justify-between">
        ...
        <Switch
          ...
          disabled={
            isLoading || (setupStep !== "method" && setupStep !== "complete")
          }
        />
      </div>
    </CardHeader>
...
const AuthenticatorApp = ({ setSetupStep, handleCancel, qrCode, setOtp }) => {
  return (
    <div className="space-y-4">
      <h3 className="font-medium">Set up authenticator app</h3>
      <ol className="list-decimal ml-4 space-y-3">
        <li>
          Download and install an authenticator app like Google Authenticator or
          Authy.
        </li>
        <li>
          Open the app and scan the QR code below or enter the setup key
          manually.
        </li>
        <li>
          Enter the 6-digit verification code from the app to verify setup.
        </li>
      </ol>
  ...
  return (
    <div className="space-y-4 disabled pointer-events-none">
      <h3 className="font-medium">
        Verify {selectedMethod === "app" ? "authenticator app" : "phone number"}
      </h3>
      <p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-6 space-y-4">
        ...
        <p className="text-sm">
          Save these recovery codes in a secure place. You can use them to sign
          in if you lose access to your
        ...
        <p className="text-sm text-muted-foreground">
          Each code can only be used once. You can generate new recovery codes
          if you run out.
        </p>
      </div>
    </div>
