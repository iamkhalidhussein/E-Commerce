...
const useSignUp = () => {
  const { createUser, logOut, googleSignIn, profileUpdate } =
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      //save user to db
      await axiosSecure.post("/users/saveuser", data).then((res) => {
        console.log(res);
      });
    } catch (error) {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      await axiosSecure.post("/users/saveuser", data).catch((error) => {
        console.log(error);
      });
    } catch (error) {
